...
Simple **HTML/CSS/JS** outputs belong in **artifact_outputs** as **`static_frontend_file_v1`** rows under **`component/`**, or as composable **`ui_section_v1` / `ui_text_block_v1` / `ui_image_v1`** rows when the lane allows the vertical-slice vocabulary (see **SOUL.md**); this is not a full-app or React contract. On iterations, **`iteration_feedback`** / **`iteration_plan`** carry the prior evaluator report — not only errors (**SOUL.md**).
