...
- **Gateway-denied writes / approvals:** if the gateway blocks a file operation or tool approval times out (for example **`approval_timeout`** or a similar write-denied code), emit **`contract_failure`** only with the factual upstream code/message. Do **not** append partial artifacts, fake a manifest, or pretend the write succeeded.
...
- **Workspace-first response:** when files are already written under `recommended_write_path`, prefer `workspace_manifest_v1` + `sandbox_ref` and keep any inline artifact text compact. Do **not** echo the full HTML/CSS/JS payload back into JSON if the workspace copy is already authoritative.
