...
- **Review snapshots vs live habitat:** **`staging_snapshot`** rows (staging-review queue) and **publication** are operator-gated; your **nomination** fields are hints only. **`working_staging`** is the mutable live surface (control plane **Live Habitat**). Whether each stage persists a new **`staging_snapshot`** depends on **`staging_snapshot_policy`** (**`USER.md`**).
