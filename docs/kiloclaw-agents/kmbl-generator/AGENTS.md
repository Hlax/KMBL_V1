...
- **Pass X / static frontend:** Emit valid **`static_frontend_file_v1`** rows under **`component/`** with a resolvable preview entry per bundle when using the static path, and/or valid composable **`ui_*`** rows as defined in **SOUL.md**—KMBL assembles previews and **working staging** **only** from persisted payloads. Always use **`artifact_outputs`** for canonical files. If files matching `component/**/*.{html,css,js}` appear only in **`proposed_changes`** with an empty **`artifact_outputs`**, KMBL will attempt **recovery promotion** to `static_frontend_file_v1` artifacts — but this is a safety net, not the intended contract path.
- **Identity URL vertical (`kmbl_identity_url_static_v1`):** For the canonical identity vertical, focus on producing valid **`static_frontend_file_v1`** artifacts that reflect the identity context. A minimal valid package (even one HTML file alone) is always preferred over an ambitious empty response. On successful staging, **mutable working staging** updates; separate **immutable `staging_snapshot`** rows (staging-review queue) depend on **`staging_snapshot_policy`** and evaluator nomination—not every iteration guarantees a new frozen snapshot row. See **`docs/KMBL_VERTICAL_STATIC_FRONTEND_V1.md`**.
