...
/**
 * GET /api/publication/current → orchestrator GET /orchestrator/publication/current
 */
export async function GET(request: Request) {
  ...
  const url = `${base}/orchestrator/publication/current${q ? `?${q}` : ""}`;
  try {
    const res = await fetch(url, { cache: "no-store" });
    const text = await res.text();
    const parsed = parseJsonSafe(text);
    if (isOrchestratorRouteNotFound(res.status, parsed.ok ? parsed.value : null, !parsed.ok)) {
      return NextResponse.json(fallbackPublicationCurrent(), { status: 200 });
    }
